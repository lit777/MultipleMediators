...
##---------------------------------------------------------------
  ...
  main <- function(temp){
      ...
      for(k in 1:3){
          ...
          index<-matrix(c(1,2,1,3,2,3),byrow=T, nrow=3)
          eval(parse(text=paste("mean.y1 <- mean(y1.sample[1,intersect(abs1",index[k,1],", pos1",index[k,2],")]); mean.y0 <- mean(y0.sample[1, intersect(abs0",index[k,1],", pos0",index[k,2],")]); SE1[",k,"] <- mean.y1-mean.y0", sep="")))
          eval(parse(text=paste("mean.y1 <- mean(y1.sample[1,intersect(neg1",index[k,1],", pos1",index[k,2],")]); mean.y0 <- mean(y0.sample[1, intersect(neg0",index[k,1],", pos0",index[k,2],")]); SE2[",k,"] <- mean.y1-mean.y0", sep="")))
          eval(parse(text=paste("mean.y1 <- mean(y1.sample[1,intersect(abs1",index[k,1],", neg1",index[k,2],")]); mean.y0 <- mean(y0.sample[1, intersect(abs0",index[k,1],", neg0",index[k,2],")]); SE3[",k,"] <- mean.y1-mean.y0", sep="")))

          eval(parse(text=paste("mean.y1 <- mean(y1.sample[1,intersect(pos1",index[k,1],", neg1",index[k,2],")]); mean.y0 <- mean(y0.sample[1, intersect(pos0",index[k,1],", neg0",index[k,2],")]); SE4[",k,"] <- mean.y1-mean.y0", sep="")))
          eval(parse(text=paste("mean.y1 <- mean(y1.sample[1,intersect(pos1",index[k,1],", abs1",index[k,2],")]); mean.y0 <- mean(y0.sample[1, intersect(pos0",index[k,1],", abs0",index[k,2],")]); SE5[",k,"] <- mean.y1-mean.y0", sep="")))
          
          eval(parse(text=paste("mean.y1 <- mean(y1.sample[1,intersect(neg1",index[k,1],", abs1",index[k,2],")]); mean.y0 <- mean(y0.sample[1, intersect(neg0",index[k,1],", abs0",index[k,2],")]); SE6[",k,"] <- mean.y1-mean.y0", sep="")))
      }
      ...
      #----- Estimating Membership Proportions
      ...
      Pr1[1] <- length(intersect(abs11,pos12)) / size
      Pr1[2] <- length(intersect(abs11,neg12)) / size
      Pr1[3] <- length(intersect(abs11,pos13)) / size
      Pr1[4] <- length(intersect(abs11,neg13)) / size
      Pr1[5] <- length(intersect(abs12,pos11)) / size
      Pr1[6] <- length(intersect(abs12,neg11)) / size
      Pr1[7] <- length(intersect(abs12,pos13)) / size
      Pr1[8] <- length(intersect(abs12,neg13)) / size
      Pr1[9] <- length(intersect(abs13,pos11)) / size
      Pr1[10] <- length(intersect(abs13,neg11)) / size
      Pr1[11] <- length(intersect(abs13,pos12)) / size
      Pr1[12] <- length(intersect(abs13,neg12)) / size
      Pr1[13] <- length(intersect(pos11,neg12)) / size
      Pr1[14] <- length(intersect(pos11,neg13)) / size
      Pr1[15] <- length(intersect(pos12,neg11)) / size
      Pr1[16] <- length(intersect(pos12,neg13)) / size
      Pr1[17] <- length(intersect(pos13,neg11)) / size
      Pr1[18] <- length(intersect(pos13,neg12)) / size
      Pr1[19] <- length(Reduce(function(x, y) intersect(x,y),list(abs11, abs12, abs13))) / size
      Pr1[20] <- length(Reduce(function(x, y) intersect(x,y),list(abs11, abs12, pos13))) / size
      Pr1[21] <- length(Reduce(function(x, y) intersect(x,y),list(abs11, abs12, neg13))) / size
      Pr1[22] <- length(Reduce(function(x, y) intersect(x,y),list(abs11, pos12, abs13))) / size
      Pr1[23] <- length(Reduce(function(x, y) intersect(x,y),list(abs11, pos12, pos13))) / size
      Pr1[24] <- length(Reduce(function(x, y) intersect(x,y),list(abs11, pos12, neg13))) / size
      Pr1[25] <- length(Reduce(function(x, y) intersect(x,y),list(abs11, neg12, abs13))) / size
      Pr1[26] <- length(Reduce(function(x, y) intersect(x,y),list(abs11, neg12, pos13))) / size
      Pr1[27] <- length(Reduce(function(x, y) intersect(x,y),list(abs11, neg12, neg13))) / size
      Pr1[28] <- length(Reduce(function(x, y) intersect(x,y),list(pos11, abs12, abs13))) / size
      Pr1[29] <- length(Reduce(function(x, y) intersect(x,y),list(pos11, abs12, pos13))) / size
      Pr1[30] <- length(Reduce(function(x, y) intersect(x,y),list(pos11, abs12, neg13))) / size
      Pr1[31] <- length(Reduce(function(x, y) intersect(x,y),list(pos11, pos12, abs13))) / size
      Pr1[32] <- length(Reduce(function(x, y) intersect(x,y),list(pos11, pos12, pos13))) / size
      Pr1[33] <- length(Reduce(function(x, y) intersect(x,y),list(pos11, pos12, neg13))) / size
      Pr1[34] <- length(Reduce(function(x, y) intersect(x,y),list(pos11, neg12, abs13))) / size
      Pr1[35] <- length(Reduce(function(x, y) intersect(x,y),list(pos11, neg12, pos13))) / size
      Pr1[36] <- length(Reduce(function(x, y) intersect(x,y),list(pos11, neg12, neg13))) / size
      Pr1[37] <- length(Reduce(function(x, y) intersect(x,y),list(neg11, abs12, abs13))) / size
      Pr1[38] <- length(Reduce(function(x, y) intersect(x,y),list(neg11, abs12, pos13))) / size
      Pr1[39] <- length(Reduce(function(x, y) intersect(x,y),list(neg11, abs12, neg13))) / size
      Pr1[40] <- length(Reduce(function(x, y) intersect(x,y),list(neg11, pos12, abs13))) / size
      Pr1[41] <- length(Reduce(function(x, y) intersect(x,y),list(neg11, pos12, pos13))) / size
      Pr1[42] <- length(Reduce(function(x, y) intersect(x,y),list(neg11, pos12, neg13))) / size
      Pr1[43] <- length(Reduce(function(x, y) intersect(x,y),list(neg11, neg12, abs13))) / size
      Pr1[44] <- length(Reduce(function(x, y) intersect(x,y),list(neg11, neg12, pos13))) / size
      Pr1[45] <- length(Reduce(function(x, y) intersect(x,y),list(neg11, neg12, neg13))) / size

    return(c(EAE1[1:7], EDE[1:7], EAE2[1:7], Pr[1:21], Pr1, SE1, SE2, SE3, SE4, SE5, SE6))
  }
  
  result<-foreach(temp = 1:n.iter, .combine = rbind) %dopar% main(temp)
  
  save(result, file="result_PCEffects.RData")
  stopCluster(cl)
